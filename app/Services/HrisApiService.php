...
class HrisApiService
{
     ...
     */
    public function fetchDirectReports(int $employid): array
    {
        try {
            $response = Http::withHeaders([
                'X-Internal-Key' => $this->key,
            ])->get("{$this->baseUrl}/api/employees/direct-reports/{$employid}");
            ...
            Log::error("HRIS fetchDirectReports exception: {$e->getMessage()}", ['emp_id' => $employid]);
            return [];
        }
    }
